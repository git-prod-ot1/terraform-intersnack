...
def get_table_name(record: dict) -> str:
    ...
    if TIMESTREAM_PARTITION_FIELD in measurement:
        table_name = f"{TIMESTREAM_TABLE_PREFIX}_{measurement[TIMESTREAM_PARTITION_FIELD]}"

    if TIMESTREAM_PARTITION_FIELD in record:
        # this is an override for factoryid in broader context
        table_name = f"{TIMESTREAM_TABLE_PREFIX}_{record[TIMESTREAM_PARTITION_FIELD]}"
    if TIMESTREAM_TABLE_SUFFIX:
        table_name = table_name + f"_{TIMESTREAM_TABLE_SUFFIX}"
...
def discard_non_compliant_measurements(record: dict) -> bool:
    measurement = record["measurement"]
    if not measurement.get("timestream", False):
        return False
...
def convert_to_timestream_obj(record):
    ...
    return {
        "Dimensions": [{"Name": "factory", "Value": dim}],
        "MeasureName": measurement["dataPointId"],
        "MeasureValue": str(val),
        "MeasureValueType": get_attribute_type(type(val).__name__),
        "Time": str(taken_at),
    }
